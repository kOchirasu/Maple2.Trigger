namespace Maple2.Trigger._02020130_bf {
    public static class _dungeonmissioncheckskillbreakclear {
        ...
        private class State던전미션_체크 : TriggerState {
            ...
            public override TriggerState? Execute() {
                if (context.CheckNpcAdditionalEffect(spawnId: 701, additionalEffectId: 70002171, level: 1)) {
                    return new State던전미션_스킬브레이크저지_Success(context);
                }

                if (context.CheckNpcAdditionalEffect(spawnId: 702, additionalEffectId: 70002171, level: 1)) {
                    return new State던전미션_스킬브레이크저지_Success(context);
                }

                if (context.CheckNpcAdditionalEffect(spawnId: 703, additionalEffectId: 70002171, level: 1)) {
                    return new State던전미션_스킬브레이크저지_Success(context);
                }
